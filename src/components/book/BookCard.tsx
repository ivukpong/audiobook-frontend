...
export default function BookCard({ book, owned }: { book: Book; owned?: boolean }) {
  const hasPlatforms = book.spotifyUrl || book.appleBooksUrl || book.googlePlayUrl || book.audibleUrl;
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:border-brand/40 hover:shadow-sm transition-all">
      <Link href={`/book/${book.id}`}>
        <div className="relative h-48 bg-brand-light overflow-hidden">
          ...
          {owned && (
            <span className="absolute top-2 right-2 bg-brand text-white text-xs font-medium px-2 py-0.5 rounded-full">Owned</span>
          )}
        </div>
        <div className="p-4">
          ...
          <div className="flex items-center justify-between mt-3">
            <span className="text-brand font-semibold">{formatPrice(book.price, book.currency)}</span>
            <span className="text-xs text-gray-400 flex items-center gap-1"><Clock size={12} />{formatDuration(book.durationSec)}</span>
          </div>
        </div>
      </Link>
      {hasPlatforms && (
